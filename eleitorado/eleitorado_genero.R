...
eleitorado_2020_n <- eleitorado_2020 %>%
  group_by(SG_UF, NM_MUNICIPIO, DS_GENERO) %>%
  ...
  mutate(total = feminino + masculino + nao_informado,
         fem_perc = (feminino / total) * 100,
         mas_perc = (masculino / total) * 100,
...
eleitorado_2016_n <- eleitorado_2016 %>%
  rename(SG_UF = V2, NM_MUNICIPIO = V3, DS_GENERO = V6, QT_ELEITORES_PERFIL = V9) %>%
