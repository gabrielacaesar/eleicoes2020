...
select_columns <- c("DT_PRESTACAO_CONTAS", "SQ_PRESTADOR_CONTAS", "NR_CNPJ_PRESTADOR_CONTA",
                    "SQ_CANDIDATO", "DS_FONTE_RECEITA", "DS_ORIGEM_RECEITA", "NM_CANDIDATO",
                    "NR_CPF_CANDIDATO",
...
receitas_candidatos <- fread("C:/Users/acaesar/Downloads/dados_12out2020/prestacao_de_contas_eleitorais_candidatos_2020/receitas_candidatos_2020_BRASIL.csv",
                             encoding = "Latin-1",
...
recursos_proprios <- receitas_candidatos %>%
  ...
  group_by(SQ_CANDIDATO, NR_CPF_CNPJ_DOADOR, NR_CPF_CANDIDATO,
           NM_DOADOR, NM_CANDIDATO) %>%
  summarise(recursos_proprios = sum(VR_RECEITA)) %>%
  mutate(check_doador = NR_CPF_CNPJ_DOADOR == NR_CPF_CANDIDATO) %>%
  filter(check_doador == TRUE) %>%
  select(SQ_CANDIDATO, recursos_proprios) %>%
